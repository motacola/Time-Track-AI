...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Call Gemini API
    logger.info("Calling Gemini API", { requestId, userId: user.id })

    const geminiApiKey = process.env.GEMINI_API_KEY
    if (!geminiApiKey) {
      logger.error("Gemini API key not configured", new Error("Missing GEMINI_API_KEY"), { requestId })
    ...
    if (!response.ok) {
      ...
      logger.error("Gemini API error", new Error(JSON.stringify(errorData)), {
        requestId,
        userId: user.id,
